...
SELECT UPPER(CONCAT(first_name, ' ', last_name)) AS full_name
FROM employees
WHERE last_name LIKE 'E%e') as subquery; -- every derived table must have its own alias

/* 5. Find all employees hired in the 90s and born on Christmas. Use datediff() function to find 
...
SELECT 
CONCAT(
LOWER(SUBSTR(first_name, 1, 1)), -- LEFT(first_name, 1)
LOWER(SUBSTR(last_name, 1, 4)),  -- LEFT(last_name, 4)
'_',
SUBSTR(birth_date, 6, 2), -- MONTH(birth_date)
SUBSTR(birth_date, 3, 2) -- SUBSTR((YEAR), 2, 2)
)AS username,
first_name,
last_name,
birth_date
